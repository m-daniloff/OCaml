...
let for_all p l =
  List.fold_left (fun result elem -> if p elem then result && true else result && false) true l;;

let exists p l =
  List.fold_left (fun result elem -> if p elem then result || true else result) false l;;
...
let sorted cmp l =
  ...
  | x::xs -> 
      let res =	(List.fold_left 
                   (fun acc  elem -> 
                      match acc with
                      | Some v when (cmp v elem) <= 0 -> Some(elem)
                      | _ -> None) 
                   (Some (x)) (xs))
      in 	
